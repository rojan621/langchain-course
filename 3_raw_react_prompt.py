...
def apply_discount(price: float, discount_tier: str) -> float:
    ...
    print(f"   >> Executing apply_discount(price={
          price}, discount_tier='{discount_tier}')")
    price = float(price)
...
2. Only call apply_discount AFTER you have received a price from get_product_price. Pass the exact price returned by get_product_price. Pass the exact price returned by get_product_price - do NOT pass a made-up number.
...
def run_agent(question: str):
    ...
    for iteration in range(1, MAX_ITERATIONS + 1):
        ...
        print(f" [Parsing] Lookinig for Final Answer in LLM output...") 
        ...
        args = [x.split("=", 1)[-1].strip().strip('"\n') for x in raw_args]

        print(f"  [DEBUG] Parsed args list: {args}")
        print(f"  [DEBUG] Calling {tool_name} with positional args: {args}")
